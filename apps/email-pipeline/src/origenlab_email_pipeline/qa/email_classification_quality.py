"""Read-only QA audit for heuristic commercial-type labels on canonical Gmail."""
...
AUDIT_HEURISTIC_BASELINE_V1: dict[str, object] = {
...
def emit_baseline_delta(summary: dict[str, object], *, file: TextIO = sys.stderr) -> None:
    """Print stderr comparison vs fixed baseline (informational)."""
    b = AUDIT_HEURISTIC_BASELINE_V1
    print("", file=file)
...
def run_audit(
    ...
    *,
    ...
    rows = fetch_sample_rows(conn, predicate_on_e=pred_canon, days=days, limit=limit)
...
def attach_audit_meta(
    ...
    *,
    ...
    payload["meta"] = {"sqlite_path": str(db_path), "days": days, "limit": limit}
    payload["baseline_comparison"] = {
        "reference": AUDIT_HEURISTIC_BASELINE_V1,
