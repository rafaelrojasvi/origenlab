"""Read-only outreach / Sent / contacted volume rollup (library).
...
class OutreachVolumeRollupResult:
    rollup_rows: list[dict[str, str]]
    summary: dict[str, Any]
    rollup_csv_path: Path
    summary_json_path: Path
    sent_email_count: int
    state_email_count: int
    marketing_union_count: int
    overlap_sent_and_state: int
    sent_not_state_count: int
    state_not_sent_count: int
...
def parse_send_manifest(path: Path) -> tuple[list[str], int, dict[str, Any]]:
...
def manifest_campaign_tag(path: Path, root: dict[str, Any]) -> str:
...
def scan_csv_emails(path: Path) -> _CsvScan:
    emails: set[str] = set()
    dates: list[str] = []
    row_count = 0
...
def cross_counts(emails: Iterable[str], sent: set[str], state: set[str]) -> tuple[int, int, int]:
    em = set(emails)
    sent_c = len(em & sent)
    st_c = len(em & state)
...
def discover_active_files(active_root: Path) -> list[tuple[str, str, Path, str]]:
...
def rollup_row_dict(
    *,
    ...
    sent_c, st_c, ov = cross_counts(unique_emails, sent_set, state_set)
    return {
...
def build_outreach_volume_rollup(
    *,
    db_path: Path,
    ...
    gmail_user: str,
    sent_folders: tuple[str, ...],
) -> OutreachVolumeRollupResult:
    """Compute rollup rows and summary; write CSV + JSON under ``out_dir``."""
    ...
    try:
        n_sent_rows, sent_emails, sent_early, sent_late = load_gmail_sent_stats(
            conn, gmail_user=gmail_user, sent_folders=sent_folders
        )
        n_state_rows, state_emails, st_early, st_late = load_outreach_blocking(conn)
    finally:
        conn.close()
    ...
    db_sent_descr = f"emails:gmail:{gmail_user}/folder_in({','.join(sent_folders)})"
    rollup_rows.append(
        rollup_row_dict(
    ...
    for kind, name, path, tag_hint in discover_active_files(reports_out_dir):
        rel = rel_path(path, repo_root)
        if kind == "send_manifest":
            try:
                raw_list, n_slots, root = parse_send_manifest(path)
                ...
                continue
            tag = manifest_campaign_tag(path, root) or tag_hint
            uniq = frozenset(raw_list)
            sent_c, st_c, _ov = cross_counts(uniq, sent_set, state_set)
            rollup_rows.append(
                {
                    "source_kind": kind,
                    "source_name": name,
                    ...
                    "earliest_date": "",
                    "latest_date": "",
                    "notes": "Recipients extracted from JSON manifest.",
                }
            )
            continue

        try:
            scan = scan_csv_emails(path)
        ...
        sent_c, st_c, ov = cross_counts(scan.emails, sent_set, state_set)
        rollup_rows.append(
            {
                "source_kind": kind,
                "source_name": name,
    ...
    write_rollup_csv(rollup_path, rollup_rows)
    ...
    return OutreachVolumeRollupResult(
        rollup_rows=rollup_rows,
        summary=summary,
        rollup_csv_path=rollup_path,
        summary_json_path=summary_path,
        sent_email_count=len(sent_set),
        state_email_count=len(state_set),
        marketing_union_count=len(marketing_union),
        overlap_sent_and_state=overlap_ss,
        sent_not_state_count=sent_not_state,
        state_not_sent_count=state_not_sent,
        top_sources=top_sources,
    )


def write_rollup_csv(path: Path, rollup_rows: list[dict[str, str]]) -> None:
    with path.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(ROLLUP_CSV_FIELDS), lineterminator="\n")
        w.writeheader()
