...
| **Lead-account (`COMPATIBILITY_WRAPPER` / `COMPATIBILITY_ONLY`)** | [`build_lead_account_rollup.py`](build_lead_account_rollup.py), [`match_lead_accounts_to_existing_orgs.py`](match_lead_accounts_to_existing_orgs.py), [`validate_lead_account_rollup.py`](validate_lead_account_rollup.py), [`audit_lead_org_quality.py`](audit_lead_org_quality.py) (under `scripts/` root) | Same behavior as `leads/advanced/*` via delegation. **Not preferred** for new operator commands. **Keep** for bookmarks and legacy one-liners until docs/tests migrate. |
...
**Root-level `COMPATIBILITY_WRAPPER` shims** — [`build_lead_account_rollup.py`](build_lead_account_rollup.py), [`match_lead_accounts_to_existing_orgs.py`](match_lead_accounts_to_existing_orgs.py), [`validate_lead_account_rollup.py`](validate_lead_account_rollup.py), [`audit_lead_org_quality.py`](audit_lead_org_quality.py) — delegate to `leads/advanced/` (same behavior). **Not preferred** for new docs or agent prompts; use **`scripts/leads/advanced/…`** instead ([`docs/SCRIPT_MAP.md`](../docs/SCRIPT_MAP.md#lead-account-scripts-canonical-vs-root-wrappers)). **Do not delete** until docs, tests ([`tests/test_critical_script_paths.py`](../tests/test_critical_script_paths.py), [`tests/test_lead_compatibility_wrappers.py`](../tests/test_lead_compatibility_wrappers.py)), and bookmarks no longer need root paths. Legacy `uv run python scripts/build_lead_account_rollup.py` (etc.) still work.
