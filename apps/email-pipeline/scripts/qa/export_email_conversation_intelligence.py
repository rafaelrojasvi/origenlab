...
def main() -> int:
    ...
    write_csv(
        out_dir / "real_client_conversations.csv",
        real_conversations,
        [
            "organization",
            "domain",
    ...
    write_csv(
        ...
        sorted(follow_ups, key=lambda r: (r["priority"], -(r["days_since_last_touch"] or 0))),
        [
            "priority",
            "organization",
            "contact_email",
    ...
    write_csv(
        ...
        sorted(opportunities, key=lambda r: (r["priority"], r["status"]), reverse=False),
        [
            "priority",
            "organization",
            "contact_email",
    ...
    write_csv(
        out_dir / "noise_and_supplier_conversations.csv",
        noise_rows,
        [
            "category",
...
Mailbox analyzed: {args.gmail_user}
DB: {db_path}
...
- Total sent emails from {args.gmail_user}: {overall["total_sent"]}
- Total received emails to {args.gmail_user}: {overall["total_received"]}
...
- If stale, run:
  - `uv run python scripts/ingest/05_workspace_gmail_imap_to_sqlite.py --gmail-user "{args.gmail_user}" --folder "INBOX" --since-days 30`
  - `uv run python scripts/ingest/05_workspace_gmail_imap_to_sqlite.py --gmail-user "{args.gmail_user}" --folder "[Gmail]/Enviados" --since-days 30`
  - `uv run python scripts/ingest/05_workspace_gmail_imap_to_sqlite.py --gmail-user "{args.gmail_user}" --folder "[Gmail]/Sent Mail" --since-days 30`
"""

    (out_dir / "email_conversation_intelligence_report.md").write_text(report_md, encoding="utf-8")
    ...
    summary_json = {
        ...
        "paths": {
            "report_md": str(out_dir / "email_conversation_intelligence_report.md"),
    ...
    if args.json_out:
        Path(args.json_out).write_text(json.dumps(summary_json, ensure_ascii=False, indent=2), encoding="utf-8")
