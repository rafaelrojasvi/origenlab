...
def _load_sent_norms_and_last_dates(
...
def _load_outreach_details(conn: sqlite3.Connection) -> dict[str, dict[str, object]]:
...
def _blocking_outreach_emails(detail: dict[str, dict[str, object]]) -> dict[str, dict[str, object]]:
...
def _build_ref_org_norms(
    ...
    *,
    ...
    if not _table_exists(conn, "lead_master"):
        return ref
    # Leads tied to blocking outreach rows
    for _em, od in outreach_block.items():
        ...
        if row and row[0]:
            ...
            if len(n) >= min_len:
                ref.add(n)
    # Leads whose mailbox appears in Sent or blocking outreach
    cur = conn.execute(
        """
...
def _norm_csv_header(raw: object) -> str:
...
def _norm_csv_scalar_value(raw: object) -> str:
...
def _norm_csv_row(raw_row: dict[object, object]) -> tuple[dict[str, str], bool]:
    out: dict[str, str] = {}
    malformed = False
    for k, v in raw_row.items():
        # DictReader may use key None for extra columns.
        if k is None:
            malformed = True
            continue
        ...
        if isinstance(v, list):
            malformed = True
            continue
        out[nk] = _norm_csv_scalar_value(v)
    return out, malformed


def _first_nonempty_csv_value(row: dict[str, str], columns: tuple[str, ...]) -> str:
    for col in columns:
        v = row.get(col)
...
def _domain_from_source_url(url_raw: str) -> str:
...
def _load_input_research_csv(path: Path) -> tuple[dict[str, str], list[dict[str, str]], dict[str, int]]:
    """Returns (pending_email_by_lead_id, raw_rows for csv-only keys)."""
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for line in reader:
            ...
            r, malformed = _norm_csv_row(dict(line))
            if malformed:
            ...
            pem = normalize_export_email(_first_nonempty_csv_value(r, _CSV_EMAIL_COLUMNS))
            if lid and pem:
                by_lead[lid] = pem
    return by_lead, raw_rows, stats


def _org_domain_for_lead(domain_norm: str | None, domain: str | None) -> str:
...
def _classify_row(
    *,
    ...
    cand_domains = _candidate_domains(org_domain, lead_em, res_em, pend_em)
    ...
    # Primary match_type (single column) by conservative priority
    match_type = ""
    matched_email = ""
    matched_domain = ""
    confidence = ""
    ...
    # (pri, match_type, matched_email, matched_domain)
    ...
    if chain:
        ...
        # Timestamps / outreach metadata for any contacted mailbox on this row
        ...
        for d in cand_domains:
            if d in contacted_domains:
                ...
                break
        if hit_dom:
            # Only hint when no exact email overlap
...
def main() -> int:
    ...
    pending_csv_stats = {"rows_scanned": 0, "malformed_rows": 0}
    if args.input_research_csv is not None:
        if not args.input_research_csv.is_file():
            ...
            return 1
        pending_by_lead, raw_csv, pending_csv_stats = _load_input_research_csv(args.input_research_csv)
        db_ids: set[str] = set()
        conn_tmp = _connect_readonly(db_path)
        try:
            if _table_exists(conn_tmp, "lead_master"):
    ...
    try:
        ...
        sent_norms, sent_last = _load_sent_norms_and_last_dates(
        ...
        outreach_all = _load_outreach_details(conn)
        outreach_block = _blocking_outreach_emails(outreach_all)
        contacted_emails = set(sent_norms) | set(outreach_block.keys())
        contacted_domains = _domains_from_emails(contacted_emails)
        ref_org_norms = _build_ref_org_norms(
        ...
        if not _table_exists(conn, "lead_master"):
            ...
            ph = ",".join("?" * len(fits))
            has_r = _table_exists(conn, "lead_contact_research")
            ...
            ORDER BY lm.id
            """
            cur = conn.execute(sql, tuple(fits))
            ...
            for lid, org_name, domain, domain_norm, email, email_norm, fit_bucket, r_em, r_dom in lead_rows:
                ...
                org_domain = _org_domain_for_lead(
        ...
        for r in csv_only_rows:
            ...
            if lid_s and lid_s in seen_lead_keys:
                continue
            org = _first_nonempty_csv_value(r, _CSV_ORG_COLUMNS)
            source_val = _first_nonempty_csv_value(r, _CSV_SOURCE_COLUMNS)
            org_domain = _org_domain_for_lead(
                r.get("resolved_domain") or r.get("organization_domain") or r.get("domain"),
                _domain_from_source_url(source_val),
            )
            pend_em = normalize_export_email(_first_nonempty_csv_value(r, _CSV_EMAIL_COLUMNS))
    ...
    any_outreach_block = sum(
        1 for r in rows_out if int(r.get("blocked_by_outreach_state", 0) or 0) == 1
    )
    same_dom = sum(1 for r in rows_out if r.get("match_type") == "same_domain_contacted")
    supp = sum(
        1
        for r in rows_out
    ...
    safe = sum(1 for r in rows_out if r.get("recommended_action") == "safe_for_gate_check")
    pending_scanned = int(pending_csv_stats.get("rows_scanned", 0))
    ...
    rows_out.sort(
        key=lambda r: (
            _action_rank(str(r.get("recommended_action", ""))),
            str(r.get("organization_name", "")),
            str(r.get("lead_id", "")),
        )
    )
    trimmed = rows_out[: int(args.limit)]
    ...
    with args.out.open("w", encoding="utf-8", newline="") as f:
        ...
        w.writeheader()
        for r in trimmed:
            out = {k: r.get(k, "") for k in _FIELDNAMES}
            for bkey in (
                "already_contacted",
                "blocked_by_sent",
                "blocked_by_outreach_state",
    ...
    topn = overlap_counter.most_common(int(args.sample_limit))
    top_s = ", ".join(f"{n}({c})" for n, c in topn) if topn else "(none)"

    print(f"Wrote {len(trimmed)} rows to {args.out}")
    print(f"total leads/research rows scanned: {total}")
    print(f"exact email already sent (match_type *_sent): {exact_sent}")
    print(f"exact email already in outreach_contact_state (match_type *_state): {exact_state}")
    print(f"rows with any outreach_contact_state block (contacted/replied/snoozed): {any_outreach_block}")
    print(f"same-domain contacted hints: {same_dom}")
    print(f"suppression hits: {supp}")
    print(f"likely safe / no overlap: {safe}")
    print(f"pending CSV rows scanned: {pending_scanned}")
    print(f"pending exact sent hits: {pending_exact_sent}")
    print(f"pending exact outreach state hits: {pending_exact_state}")
    print(f"pending suppressed hits: {pending_suppressed}")
    if pending_scanned:
        print(f"pending malformed CSV rows ignored: {int(pending_csv_stats.get('malformed_rows', 0))}")
    print(f"top {args.sample_limit} organizations with overlap: {top_s}")
    return 0
