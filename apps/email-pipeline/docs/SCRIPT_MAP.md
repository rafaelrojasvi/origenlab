...
| `scripts/mart/build_email_mart_features.py` | superseded_by_origenlab | `origenlab build-email-mart-features` | SQLite raw | SQLite `email_mart_features` | medium | `uv run origenlab build-email-mart-features` | **Dry-run** default; **`--apply`** writes missing/stale rows; not wired into build-mart or daily-core |
