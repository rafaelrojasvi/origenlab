...
| **Commercial intel** | [`commercial/`](../../src/origenlab_email_pipeline/commercial/) — `commercial_intel_schema.py`, `commercial_intel_queries.py`, `commercial_intel_review.py`, `commercial_intel_rules.py` (root `commercial_intel_*.py` = **shims** re-exporting the same modules) | **Core** for Engine-B review/suppression semantics on top of SQLite; optional for installs that never built commercial tables. |
...
6. **Commercial intel imports** — Prefer **`origenlab_email_pipeline.commercial.commercial_intel_*`** (or the root **shim** path) for schema/queries/rules/review; do not duplicate SQL or review semantics outside this cluster.
