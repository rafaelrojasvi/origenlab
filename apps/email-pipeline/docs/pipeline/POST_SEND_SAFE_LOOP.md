...
| **Canonical tool** | [`flag_ndr_bounces_from_contacto.py`](../../scripts/tools/flag_ndr_bounces_from_contacto.py) — NDR dry-run by default; add `--include-reported-non-delivery` for inbound human «no recibimos su correo» replies; then targeted `--emails-file` + `--only-code` + `--apply`. |
