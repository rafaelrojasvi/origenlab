...
**See also:** `uv run origenlab operator-automation-status` — read-only health for both automation loops (mail + mirror). Also visible on dashboard Today via `GET /operator/automation-status` (apps/api).
